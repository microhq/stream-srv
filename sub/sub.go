...
func NewDispatcher(id int64, size int) (Dispatcher, error) {
	// bufferred message channel
	in := make(chan *pb.Msg, size)
...
func (d *dispatcher) Start(wg *sync.WaitGroup) {
	...
	for {
		select {
		case <-d.done:
			log.Logf("Stopping dispatcher for stream: %d", d.id)
			return
		case msg := <-d.in:
			log.Logf("Dispatching message to subscribers on stream: %d", d.id)
...
func (d *dispatcher) Dispatch(msg *pb.Msg) error {
	d.in <- msg
	return nil
}
